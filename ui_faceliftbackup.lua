...
local function build_preview_cards(node)
    ...
    wrap_into(summary, safe(info.plain_english, 'Select a set or category to inspect it.'), cfg.row_fg_set, width)

    add_section(summary, 'Summary')
    if info.category then wrap_into(summary, 'Category: ' .. tostring(info.category), cfg.row_fg_set, width) end
    if info.trigger then wrap_into(summary, 'Trigger: ' .. tostring(info.trigger), cfg.row_fg_set, width) end
    if info.purpose then wrap_into(summary, 'Purpose: ' .. tostring(info.purpose), cfg.row_fg_set, width) end
    ...
    add_section(summary, 'Gear')
    wrap_into(summary, tostring(#gear) .. ' direct gear slot(s) defined in this set.', cfg.row_fg_set, width)
...
local function update_positions()
    ...
    local tab_bar_x = d.preview_x
    local panel_rendered_w = rendered_width(state.tree_bg, d.total_w)
    local char_w = panel_rendered_w / math.max(1, cols(d.total_w))
    local tab_bar_w = math.max(d.preview_w, panel_rendered_w - (tab_bar_x - d.x))
    local tab_w = math.floor(tab_bar_w / tab_count)

    for index, tab in ipairs(tabs) do
        local x = tab_bar_x + ((index - 1) * tab_w)
        local width = tab_w
        if index == tab_count then
            width = tab_bar_w - ((index - 1) * tab_w)
        end
        local active = state.preview_card == tab.card
        tab.obj:pos(x, d.body_y)
        tab.obj:bg_visible(true)
        set_title_active(tab.obj, active)
        tab.obj:text(center_pad(tab.label, math.max(#tab.label, math.ceil(width / char_w))))
    end
...
function ui.toggle_preview_focus()
    ...
    state.source_pan = 0
    clear_source_left_guard()
    state.preview_lines = state.preview_cards.gear or {}
    if state.visible then ui.refresh() end
end
